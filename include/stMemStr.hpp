...
namespace String {
	...
	static const wchar_t st_eosW = L'\0';
	static const char st_eosA = '\0';

	// eos is End of String
	template<typename type_of_char> ST_INLINE type_of_char st_eos() { return NULL; }
	template<> ST_INLINE wchar_t st_eos() { return st_eosW; }
	template<> ST_INLINE char st_eos() { return st_eosA; }
	...
	//
	// strLen
	template<typename Type> un32 strLen( const Type *p );
	...
	// strLen
	// strCpy
	template<typename Type> void strCpy( typename Type **pp, const Type *cp);
	...
	// strCat
	template<typename Type> void strCat( typename Type **pp, const Type *cp);
	...
	// strCmp
	template<typename Type> bool strCmp( Type *p, Type *cp);
	...
	// sprintf
	template<typename Type, typename type_of_digit> type_of_digit sprintf_( Type *p, type_of_digit dg );
	...
	// Conversations
	template<typename Type, typename type_of_digit> type_of_digit StrToDigit( const Type *p );
	...
	n32 FindChar( const type_of_char *psrc, const type_of_char key, const un rank, const stLib::ST_FrontOrBack flag ) {
		...
		if( flag == ST_FrontOrBack::Both || flag == ST_FrontOrBack::Neither ) {
			return -1;
		}
		...
		if (flag == ST_FrontOrBack::Front) {
			...
			for (; ( *str != st_eos<type_of_char>() ); ++str) {
				if (*str == key) {
	...
	void SelfAppend( typename type_of_char **ppsrc ) {
		...
		*( *ppsrc + ( ognLen * 2 ) ) = ( st_eos<type_of_char>() );
	}

	template<typename type_of_char>
	void TryExtendMem( typename type_of_char **ppsrc, const type_of_char *extra ) {
		const un32 curSize = st_allocator.msizeOf( *ppsrc );
	...
	bool IsAWiderThanB( const type_of_char *pA, type_of_char *pB ) {
		return strLen( pA ) > st_allocator.msizeOf( pB );
	}

	template<typename type_of_char>
    typename type_of_char * 
	NewAndCpy( typename type_of_char **ppalloc, const type_of_char *cpySrc ) {
		*ppalloc = st_allocator.NewArr<type_of_char>( strLen( cpySrc ) + 1 );
	...
	New( typename type_of_char **ppalloc ) {
		*ppalloc = st_allocator.NewArr<type_of_char>( un_new_size_at_the_first_time );
    ...
    typename type_of_char *	
	New() {
		type_of_char *alloc = st_allocator.NewArr<type_of_char>( un_new_size_at_the_first_time );
	...
	Fit( typename type_of_char **ppsrc )  {
		...
		*ppsrc = st_allocator.NewArr<type_of_char>( strlen( *ppsrc ) + 1 );
		strCpy( *ppsrc, src );
		SafeDel( &src );
		return *ppsrc;
	}
	...
	Extend( typename type_of_char **ppsrc, un32 newSize ) {
		...
		*ppsrc = st_allocator.NewArr<type_of_char>( newSize );
		ZeroMem( ppsrc );
		strCpy( ppsrc, src );
		SafeDel( &src );
		return *ppsrc;
